...
func (a *App) cmdLeave(ctx context.Context, m *tgbotapi.Message) {
	active, err := a.store.ActiveGame(ctx, m.Chat.ID)
	if err != nil || active.Game.Status != poker.StatusWaiting {
		a.reply(m.Chat.ID, m.MessageID, "当前没有等待中的牌局。")
		return
	}
...
func (a *App) handleCallback(ctx context.Context, q *tgbotapi.CallbackQuery) {
	...
	case "join":
		...
		a.cmdJoin(ctx, msg)
	case "begin":
		if game.CreatorID != q.From.ID && !a.cfg.IsAdmin(q.From.ID) {
...
func (a *App) handleDeadlines(ctx context.Context) {
	...
	if err == nil {
		for _, active := range waiting {
			lock := a.lockFor(active.Game.ChatID)
			lock.Lock()
			fresh, err := a.store.ActiveGame(ctx, active.Game.ChatID)
			if err == nil && fresh.Game.Status == poker.StatusWaiting && !fresh.ActionDeadline.After(now) {
	...
	for _, active := range running {
		...
		lock.Lock()
		fresh, err := a.store.ActiveGame(ctx, active.Game.ChatID)
		if err == nil && fresh.Game.Status == poker.StatusRunning && !fresh.ActionDeadline.After(now) {
